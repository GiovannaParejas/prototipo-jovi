  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      reconhecerComGemini(imagemBase64)
        .then(texto => {
          msg.remove();
          textoOCR = typeof texto === 'string' ? texto : texto.map(l => l.texto).join('\n');
          ...
          `;
          btnCopiar.onclick = () => {
            navigator.clipboard.writeText(textoOCR).then(() => mostrarAviso('Tudo copiado!'));
          };
...
reconhecerComGemini(imagemBase64, 'traduzir')
  .then(textoTraduzido => {
    ...
    `;
    btnCopiar.onclick = () => {
      navigator.clipboard.writeText(textoTraduzido).then(() => mostrarAviso('Tradução copiada!'));
    };
    botoes.appendChild(btnCopiar);
    wrapper.appendChild(botoes);
  })
