...
// --- Digitalização real de documento (jscanify + OpenCV.js) ---
let openCvPronto = false;
const scannerDocumento = typeof jscanify !== "undefined" ? new jscanify() : null;

function onOpenCvCarregado() {
...
function aguardarOpenCvPronto(timeoutMs = 8000) {
  return new Promise((resolve) => {
    if (openCvPronto && scannerDocumento) {
      resolve(true);
      return;
    }
...
function aplicarRealceDeDigitalizacao(canvasOrigem) {
  const canvasRealcado = document.createElement("canvas");
  canvasRealcado.width = canvasOrigem.width;
  canvasRealcado.height = canvasOrigem.height;
  const ctx = canvasRealcado.getContext("2d");
  ctx.filter = "contrast(1.5) brightness(1.1) saturate(0.4)";
  ctx.drawImage(canvasOrigem, 0, 0);
  return canvasRealcado;
}
...
function detectarDocumento(canvasCaptura) {
  const img = cv.imread(canvasCaptura);
  try {
    const contorno = scannerDocumento.findPaperContour(img);
    if (!contorno) return null;

    const corners = scannerDocumento.getCornerPoints(contorno);
    const { topLeftCorner, topRightCorner, bottomLeftCorner, bottomRightCorner } = corners;
    if (!topLeftCorner || !topRightCorner || !bottomLeftCorner || !bottomRightCorner) {
      return null;
    }

    // usa as proporções reais do documento detectado, em vez de um tamanho fixo,
    // para evitar distorcer o resultado ao forçar um formato de página arbitrário
    const largura = Math.round(
      (distanciaEntrePontos(topLeftCorner, topRightCorner) +
        distanciaEntrePontos(bottomLeftCorner, bottomRightCorner)) /
        2,
    );
    const altura = Math.round(
      (distanciaEntrePontos(topLeftCorner, bottomLeftCorner) +
        distanciaEntrePontos(topRightCorner, bottomRightCorner)) /
        2,
    );
    if (largura < 1 || altura < 1) return null;

    return scannerDocumento.extractPaper(canvasCaptura, largura, altura, corners);
  } finally {
    img.delete();
  }
}
...
async function capturarEDigitalizar(video, wrapper) {
  ...
  msg.textContent = "Preparando digitalização...";
  const opencvOk = await aguardarOpenCvPronto();

  let canvasDocumento = canvasCaptura;
  if (opencvOk) {
    try {
      const canvasDetectado = detectarDocumento(canvasCaptura);
      if (canvasDetectado) canvasDocumento = canvasDetectado;
    } catch (err) {
      console.error("Erro ao detectar documento na foto:", err);
    }
  }

  const canvasFinal = aplicarRealceDeDigitalizacao(canvasDocumento);
  const imagemCapturada = canvasFinal.toDataURL("image/png");
